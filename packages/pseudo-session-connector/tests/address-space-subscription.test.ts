...
import { DataType, Variant, type UAVariable } from 'node-opcua';
import { afterAll, beforeAll, describe, expect, it } from 'vitest';
import {
  AddressSpaceMonitoredSubscription,
} from '../src/address-space-subscription.js';
import {
  createTestContext,
  type TestContext,
  teardownTestContext,
...
describe('AddressSpaceMonitoredSubscription', () => {
  ...
  function setTemp(value: number): void {
    const v = ctx.addressSpace.findNode(
  ...
  it('fires callback on value change and monitors multiple '
    + 'variables independently', async () => {
      const sub = new AddressSpaceMonitoredSubscription(
        ctx.addressSpace, consoleLogger,
      );
      await sub.addItems([
        ctx.nodeIds.temperature,
      ...
      // Mutate both
      setTemp(77.7);
      const pressVar = ctx.addressSpace.findNode(
        ctx.nodeIds.pressure,
      )! as UAVariable;
      pressVar.setValueFromSource(
        new Variant({ dataType: DataType.Double, value: 20 }),
      );

      expect(received).toHaveLength(2);
      expect(received.map((r) => r.nodeId)).toContain(
        ctx.nodeIds.temperature,
      );
      expect(received.map((r) => r.nodeId)).toContain(
        ctx.nodeIds.pressure,
      ...
      pressVar.setValueFromSource(
        new Variant({
          dataType: DataType.Double, value: 101.3,
        }),
      );
    });

  it('stops firing after removeItems() and after close()',
    async () => {
      const sub = new AddressSpaceMonitoredSubscription(
        ctx.addressSpace, consoleLogger,
      );
  ...
  it('does not duplicate listeners on repeated addItems()',
    async () => {
      const sub = new AddressSpaceMonitoredSubscription(
        ctx.addressSpace, consoleLogger,
      );
